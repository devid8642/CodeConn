from users.forms import LoginForm, RegisterForm, UpdateForm, UpdatePasswordForm
...
class TestLoginForm(TestBase):
    def test(self):
        form = LoginForm(
            initial={'email': 'aaaaaaaaaaaaaaaaaaa', 'password': ''}
        )
...
class TestUpdateForm(TestBase):
    ...
    def test_with_exist_email(self):
        form = UpdateForm(
            initial={'username': 'teste', 'email': self.user.email}
        )
...
class TestUpdatePasswordForm(TestBase):
    def test_with_week_password(self):
        form = UpdatePasswordForm(
            initial={'password': 'teste', 'new_password': '123'}
        )
